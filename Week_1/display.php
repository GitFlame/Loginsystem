      ...
      <tbody>
        ...
        if ($result) {
          while ($row = mysqli_fetch_assoc($result)) {
            ...
            $statusClass = ($row['is_active'] == 1) ? 'btn-success' : 'btn-warning';
            ?>
            <tr>
              ...
              <td>
                ...
                <button class="btn <?php echo $statusClass; ?> toggle-status" data-id="<?php echo $id; ?>"
    ...
    function updateUserStatus(userId, newStatus) {
      $.ajax({
        ...
        success: function (response) {
          if (response.success) {
            // Update the button appearance without a page refresh
            var button = $('[data-id="' + userId + '"]');
            if (newStatus === 1) {
          ...
          success: function (response) {
            if (response.status === 'success') {
              ...
              if (newStatus === 1) {
                ...
                button.addClass('btn-warning').text('Inactive');
              }
